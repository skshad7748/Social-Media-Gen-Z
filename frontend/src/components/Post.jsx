...
const Post = ({ post }) => {
    ...
    const likeOrDislikeHandler = async () => {
        try {
            ...
            const res = await axios.get(`https://social-media-gen-z-2.onrender.com/api/v1/post/${post._id}/${action}`, { withCredentials: true });
    ...
    const bookmarkHandler = async () => {
        try {
            const res = await axios.get(`http://localhost:5173/api/v1/post/${post?._id}/bookmark`, {withCredentials:true});
            if(res.data.success){
                toast.success(res.data.message);
            }
        } catch (error) {
